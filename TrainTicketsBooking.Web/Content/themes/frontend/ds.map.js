...
function drawMarkersInBounds() {
    ...
    if (map.getZoom() < 15) {   //建议标注街道、商圈
        ...
        $.post("/Search/SearchBoardInBounds", "swLng=" + sw.lng + "&swLat=" + sw.lat + "&neLng=" + ne.lng + "&neLat=" + ne.lat + "&size=" + size + "&grade=" + grade + "&status=" + status + "&level=2", function (data) {
        ...
        $.post("/Search/SearchBoardInBounds", "swLng=" + sw.lng + "&swLat=" + sw.lat + "&neLng=" + ne.lng + "&neLat=" + ne.lat + "&size=" + size + "&grade=" + grade + "&status=" + status + "&level=4", function (data) {
            if (data.geoms && data.geoms.length > 0) {
                var s = [];
                s.push('<ul class="nav"><li class="has-sub active"><a href="javascript:;"><i class="fa fa-th"></i> 区域找到123条记录</a><ul class="sub-menu">');
...
BoardMarker.prototype.initialize = function (map) {
    this._map = map;
    var markerDiv = $("<div class='marker marker_hover' id='" + this._id + "' name='" + this._name + "' count='" + this._count + "' style='display:block'></div>")
